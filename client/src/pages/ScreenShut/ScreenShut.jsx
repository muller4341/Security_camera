...
const ScreenShut = () => {
    ...
    return (
        ...
        <div className='flex mt-10 flex-wrap justify-center bg-yellow-100 rounded-lg h-3/4 w-5/6 items-center bg-red-900'>
            <div className=" grid grid-cols-2 gap-4   bg-gray-400">
                {data.map((item,i) => {
                    <div key={i} className={`relative ${i === 0 ? 'animate-zoom-in-out' : ''}`}>
                    <img src={item.url} alt={`Screenshot ${item.id}`} className="w-full h-auto" />
                    <p className="absolute bottom-0 left-0 bg-black bg-opacity-50 text-white p-1">{item.cameraName}</p>
                </div>
